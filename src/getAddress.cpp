...
BLDCDriver3PWM RIGHTdriver = BLDCDriver3PWM(32,33,25);

BLDCMotor LEFTmotor = BLDCMotor(11);
BLDCDriver3PWM LEFTdriver = BLDCDriver3PWM(26,27,14);
...
void initialize() {
  ...
  RIGHTsensor.init(&I2Cone);
  LEFTsensor.init(&I2Ctwo);
  ...
  LEFTmotor.velocity_limit = 110;

  Serial.begin(9600);
  RIGHTmotor.useMonitoring(Serial);
  LEFTmotor.useMonitoring(Serial);

  RIGHTmotor.sensor_direction = CW;
  LEFTmotor.sensor_direction = CCW;
...
void updateZero() {
  zeroOffsetR = RIGHTsensor.getAngle();
  zeroOffsetL = LEFTsensor.getAngle();
}
...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (incomingInformation.command == CMD_INIT) {
    Serial.println("Initializing Motors and Sensors");
    initialize();
    return;
  }
...
//if true then move motors
  ...
  RIGHTmotor.move(target - zeroOffsetR);
